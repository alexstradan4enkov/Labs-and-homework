...
int main() {
	...
	cout << "Никогда не опускался ниже 5%: " << (never_low ? "Да" : "Нет") << endl;
...
int main() {
	...
	cout << "Суммарное: " << total << " кВт·ч" << endl;
	cout << "Среднее: " << avg << " кВт·ч" << endl;

	auto peak = max_element(consumption.begin(), consumption.end());
	cout << "Пиковое потребление: " << *peak << " кВт·ч" << endl;
...
int main() {
	...
	cout << "Самый горячий канал: " << distance(avg_temps.begin(), hottest) + 1 << endl;
